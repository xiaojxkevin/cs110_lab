...
int main () {
  ...
  printf ("No parallel solution with %d repeats took %f seconds\n\n", REPEAT, run_time);
  ...
  for (int i = 1; i <= num_threads; i++)
  {
    ...
    run_time = omp_get_wtime() - start_time;
    printf ("Manual Optimized with %d repeats: %d thread(s) took %f seconds\n", REPEAT, i, run_time);
  ...
  for (int i = 1; i <= num_threads; i++)
  {
    ...
    run_time = omp_get_wtime () - start_time;
    printf ("Reduction Optimized with %d repeats: %d thread(s) took %f seconds\n", REPEAT, i, run_time);
  ...
  printf ("Naive with %d repeats: %d thread(s) took %f seconds\n", REPEAT, 1, run_time);
